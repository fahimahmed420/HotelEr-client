          ...
          {activeSection === "profile" && (
            <div className="relative bg-white/60 backdrop-blur-lg p-8 rounded-2xl shadow-xl max-w-4xl mx-auto min-h-full transition">
              ...
              <div className="flex items-center space-x-4 mb-8 relative">
                ...
                {showAvatarActions && (
                  <div className="space-x-2 flex items-center">
                    ...
                    {newAvatar && (
                      <button className="px-4 py-2 border border-gray-300 rounded hover:bg-gray-100 transition" onClick={() => { setAvatar(newAvatar); setNewAvatar(null); }}>
                        Save Avatar
                      </button>
                    )}
                  </div>
              ...
              <form className="grid grid-cols-2 gap-4 text-sm">
                <div><label>First Name</label><input className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" defaultValue={firstName} disabled={!isEditable} /></div>
                <div><label>Last Name</label><input className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" defaultValue={lastName} disabled={!isEditable} /></div>
                <div>
                ...
                <div><label>Phone</label><input className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" defaultValue={user?.phoneNumber || ""} disabled={!isEditable} /></div>
                <div><label>City/State</label><input className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" defaultValue={user?.location || ""} disabled={!isEditable} /></div>
                <div><label>Country</label><select className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" disabled={!isEditable}><option>{user?.country || "USA"}</option></select></div>
                <div><label>Postal Code</label><input className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" defaultValue={user?.postalCode || ""} disabled={!isEditable} /></div>
                <div><label>Tax ID</label><input className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" defaultValue={user?.taxId || ""} disabled={!isEditable} /></div>
                <div><label>Company Name</label><input className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" defaultValue={user?.company || ""} disabled={!isEditable} /></div>
                <div><label>Language</label><select className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" disabled={!isEditable}><option>{user?.language || "English"}</option></select></div>
                <div className="col-span-2"><label>Bio</label><textarea rows="4" className="w-full p-2 rounded mt-1 bg-gray-50 shadow-inner" defaultValue={user?.bio || "Washim Chowdhury is a dedicated truck owner with 10 years of experience in transportation and logistics..."} disabled={!isEditable} /></div>
              </form>
              ...
              {isEditable && (
                <div className="flex justify-end mt-6 space-x-2">
                  ...
                  <button className="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700 transition">Save</button>
                </div>
              )}
            </div>
          )}
